...
class CycleCols:
    ...
    VOLTAGE = 4
    CURRENT = 5
    DISCHARGING_CAPACITY = 7
    TEMPERATURE = 10
    SOC = 12
    REMAINING_TIME_TO_CYCLE_END = 13
...
class UniboPowertoolsData():
    ...
    def __filter_raw_chunk(self, iter_chunk):
        ...
        for chunk in iter_chunk:
            mask = pd.Series(True, index=chunk.index)  # Start with all True
        # Apply test_types filter
            if self.test_types:
                mask &= chunk['test_name'].str.endswith(tuple(self.test_types))
        # Apply lines filter
            if self.lines:
                mask &= chunk['line'].isin(self.lines)
            filtered_chunk = chunk[mask]
            filter_cks.append(filtered_chunk)
        return filter_cks
    ...
    def __get_cyc_and_cap(self, names, cyc_raw, cap_raw):
        ...
        for test_name in names:
            ...
            for cycle in gp_cyc_raw[test_name]:
                ...
                # Select only numerical columns from cycle
                numerical_cycle = cycle.select_dtypes(include=[np.number])
                cyc_data.append(numerical_cycle.values)
                cap_data.append(target_cap_row.values)

                if len(numerical_cycle) > max_length:
                    max_length = len(numerical_cycle)

        padded_cyc_data = np.zeros((len(cyc_data), max_length, cyc_data[0].shape[1]))
        for i, cycle in enumerate(cyc_data):
            padded_cyc_data[i, :len(cycle), :] = cycle

        cap_data = np.array(cap_data)
        padded_cyc_data = np.array(padded_cyc_data)

        return (padded_cyc_data, cap_data)
    ...
    def __add_discharge_soc_pars(self, discharge_cyc, charge_cap):
        # Create a new list to store the updated discharge cycles
        updated_discharge_cyc = []
        for i in range(len(discharge_cyc)):
            # Get the current discharge cycle
            cycle = discharge_cyc[i]
            # Create a new array with space for two additional columns (SOC and Time Remaining)
            updated_cycle = np.zeros((cycle.shape[0], cycle.shape[1] + 2))  # Add 2 columns
            updated_cycle[:, :-2] = cycle  # Copy the original data into the new array
            # Calculate SOC and store it in the second-to-last column
            soc = (
                charge_cap[i][CapacityCols.CHARGING_CAPACITY] - cycle[:, CycleCols.DISCHARGING_CAPACITY]
            ) / charge_cap[i][CapacityCols.CHARGING_CAPACITY]
            updated_cycle[:, -2] = soc  # Store SOC in the second-to-last column
            
            # Calculate Time Remaining and store it in the last column
            time_remaining = cycle[-1, CapacityCols.TIME] - cycle[:, CycleCols.TIME]
            updated_cycle[:, -1] = time_remaining  # Store Time Remaining in the last column

            # Append the updated cycle to the list
            updated_discharge_cyc.append(updated_cycle)
        updated_discharge_cyc=np.array(updated_discharge_cyc)
        return updated_discharge_cyc
